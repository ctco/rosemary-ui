...
class DaySlot extends React.Component {
    ...
    renderEvents() {
        ...
        return events.map((event, idx) => {
            ...
            lastLeftOffset = Math.max(0, overlaps(event, events.slice(0, idx), this.props, lastLeftOffset + 1));
            ...
            if (eventPropGetter) var { style: xStyle, className } = eventPropGetter(event, start, end, _isSelected); //eslint-disable-line
    ...
    render() {
        const { min, max, step, timeslots, now, selectRangeFormat, culture, ...props } = this.props;
...
DaySlot.propTypes = PROPERTY_TYPES;
