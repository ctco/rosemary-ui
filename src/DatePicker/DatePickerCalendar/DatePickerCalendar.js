...
import { formatD, formatDDMMYYYY, parseDDMMYYYY, formatWeekDay } from '../../util/date-formats';
...
class DatePickerCalendar extends React.Component {
    ...
    renderDaysOfWeek() {
        ...
        for (let i = 1; i <= 7; i++) {
            ...
            result.push(
                <td key={formattedWeekday} className={className}>
                    {formattedWeekday}
                </td>
            );
        }
    ...
    renderWeek(week) {
        return week.map(rollable => {
            ...
            if (!this.props.renderDatesOfOtherMonth && !isSameMonth) {
                ...
                return (
                    <td {...props}>
                        <div>{this.formatDate(rollable)}</div>
                    </td>
                );
            }
        });
